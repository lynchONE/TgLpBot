...
export async function fetchSMPools({ apiBaseUrl, page = 1, size = 10, keyword, protocol, source, minSmartMoneyUsd, maxFeeRate, minMarketCapUsd, signal }) {
    ...
    if (source) params.set('source', source);
    if (Number.isFinite(minSmartMoneyUsd) && minSmartMoneyUsd > 0) params.set('min_smart_money_usd', String(minSmartMoneyUsd));
    if (Number.isFinite(maxFeeRate) && maxFeeRate > 0) params.set('max_fee_rate', String(maxFeeRate));
    if (Number.isFinite(minMarketCapUsd) && minMarketCapUsd > 0) params.set('min_fdv_usd', String(minMarketCapUsd));
...
export async function recalculateSMAutoFollowPnL({ apiBaseUrl, initData, chain = 'bsc', id, signal }) {
    const base = normalizeBase(apiBaseUrl);
    return goldenDogRequest(`${base}/api/smart_money_auto_follow`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ initData, chain, action: 'recalculate_pnl', id }),
        signal,
    });
}
