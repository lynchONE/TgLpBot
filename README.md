...
This starts only the middleware services from the repo root: MySQL, Redis, and MinIO.

```bash
docker compose up -d
```

To change middleware ports or passwords, copy `docker-compose.env.example` to `.env` in the repo root and edit it.
...
If the backend runs on the host machine, point `backend/.env` at the published middleware ports, for example `MYSQL_HOST=127.0.0.1`, `REDIS_HOST=127.0.0.1`, and `MINIO_ENDPOINT=127.0.0.1:9000`.
...
## 2026-04 钱包增删改查与兑换 UI 重构升级

### 1. 钱包管理模块升级
- **增删改查支持**：新增功能让你能通过 WebApp 或 Miniapp 界面直接私钥导入钱包、新建、删除、修改名称、设定默认选项。
- **后台统一管理**：采用安全 API 调用后端交互控制钱包，所有私密内容依然由后端 AES 限流加密。

### 2. "我的" 导航重命名
- 将全站原“管理”统一更名为符合主流 Web3 应用通用习惯的“我的”。

### 3. Uniswap 风格兑换页面重构
- **废弃全遍历扫描**：完全移除了旧版的一键扫描并强制批量变卖全包代币功能。
- **单代币精准兑换**：重头构建基于输入的精确兑换模式，UI 设计类似 Uniswap 和主流去中心化交易所。
- **防抖实时报价**：通过复用 OKX Dex 后端聚合器路由，实现了高效流畅的动态询价（Quote）与兑换（Swap）。同时支持任意自定义滑点及自选支付钱包操作。

## 2026-04 开仓界面布局与交互体验优化

### 1. 结构与流线重构
- 痛点：以往弹窗中，“开仓金额”的输入框和“加仓建议”区块相隔过远，用户在阅读建议后需要折返向上输入，视觉流不顺畅。
- 修复：大幅调整了 WebApp 与 MiniApp 中 `OpenPositionModal` / 页面内的布局排版，将“加仓建议”模块直接上提至“开仓金额”组件的正下方。

### 2. 快捷建议一键填入 (Tap-to-Fill)
- 痛点：原加仓建议呈现形式为静态组件，仅仅用于“展示与提示”用途，用户依旧需要记住数值并手动敲击键盘输入。
- 修复：双端均重构了建议卡片的逻辑，为其增加了智能的回填能力与点击事件。现在，当用户点击任一加仓建议（保守 / 中性 / 激进）时，系统会自动清空先前的验证错误，并将对应的金额数值精准且无缝地填入“开仓金额”输入框中，极大加速了移动端的开仓速度。
- UI升级：赋予了卡片按压反馈缩小 (`active:scale-95`) 与悬停亮化 (`hover:brightness-110`) 的过渡动画质感。

### 3. 极简胶囊化与去框扁平架构 (Flat & Pill UI)
- 痛点：原本加仓建议卡片占据极大高度，并且各类输入模块周围充斥着过度冗余的 `border` 和 `background`（盒中盒），显得页面拥挤且厚重。
- 修复：大幅砍去了表单内各模块厚重封闭的边缘框和内边距，统一采取 Flat（扁平底）衔接技术；并完美把加仓建议的九宫格视图折叠收缩为了诸如 `[🛡️保守 $1.5K]` 字样的一行极简**胶囊按钮 (Pill)** 排列，极大释放并净化了开仓界面的有效可视纵界。
