...
### Position manager config (recommended)

`ZapSimple` restricts funds-moving entry points to the deploying wallet. External swap targets are not configured on-chain.  
The deploy script auto-calls `setPositionManagers` from env values.
