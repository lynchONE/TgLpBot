// Smart Money golden dog config API proxy
// Forwards requests to backend: GET/POST /api/smart_money_golden_dog_config
...
export default async function handler(req, res) {
    ...
    const url = `${backendBaseUrl}/api/smart_money_golden_dog_config${method === 'GET' ? buildQueryString(req.query) : ''}`;
    ...
    try {
        ...
        if (upstream.ok && !body) {
            ...
            const chain = String(req.query?.chain || req.body?.chain || 'bsc');
