...
- 再增加“池子参数模式”
- 为每种模式增加通知强度与测试入口
- 为聪明钱聚集模式增加 LP 金额阈值，过滤只有数量但资金力度不足的信号
- 在聪明钱聚集模式中支持按命中金额自动选择 Bark 强度
...
- Non-Goals:
  ...
  - 不在第一版中支持按单个钱包、单个仓位或任意自定义表达式做金额阶梯
...
- Decision: 聪明钱聚集模式的金额阈值使用交易对统计窗口内的 LP USD 金额合计。
  - Why: 金狗通知当前按交易对聚合，多钱包小额噪音应该在同一个聚合口径上被过滤；用户看到的也是“这个交易对本轮聪明钱资金力度”。
  - 金额来源：
    - 优先使用 `sm_lp_events.total_usd`
    - 为空时使用 `token0_amount_usd + token1_amount_usd`
    - 仍缺失时按 0 金额参与合计
  - `min_total_amount_usd <= 0` 表示关闭金额阈值，兼容旧行为。

- Decision: 金额阶梯强度以本次交易对信号的 `total_amount_usd` 作为匹配值。
  - Why: 这样配置最直观，低金额普通提醒，高金额提高提醒强度。
  - 阶梯结构：`min_amount_usd + intensity`。
  - 命中规则：按 `min_amount_usd` 升序校验，选择最高命中档；无命中时回退到固定强度。
  - 第一版仅在聪明钱聚集模式启用金额阶梯；池子参数模式继续使用固定强度，避免不同池子指标之间的阶梯口径混淆。
...
- 历史 LP 事件可能缺少 USD 金额，直接按金额阈值过滤会减少可触发样本。
  - Mitigation: 金额阈值默认关闭；启用后缺失金额按 0 处理，并在 UI 摘要里提示“仅统计可估值金额”。

## Migration Plan
1. 扩展 `smart_money_golden_dog_configs`，把现有字段视为“聪明钱聚集模式”字段并补齐强度字段、金额阈值字段与金额阶梯强度配置字段。
2. 为池子参数模式新增一组字段，默认全部关闭/为空，避免对现有用户产生行为变化。
3. Bark 强度默认值统一为 `ring`，保证老用户升级后行为接近当前版本。
4. 金额阈值默认值为 0，金额阶梯默认关闭，保证老用户升级后不会因为金额缺失而漏推。
5. 告警状态表继续沿用，新增模式后统一按带前缀的 key 写入。
6. 前端读取新接口结构后，WebApp / MiniApp 页面一起切到双模式视图，并在聪明钱聚集模式中展示金额阈值与阶梯强度配置。
