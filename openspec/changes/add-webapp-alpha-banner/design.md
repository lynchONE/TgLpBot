...
- Goals:
  ...
  - 今日空投可一键开启 Bark 提醒，默认在空投时间前 3 分钟推送。
- Non-Goals:
  - 不在本次实现完整稳定度详情页。
  - 不把 Alpha 数据写入后端数据库。
...
- Decision: 空投提醒配置复用用户 `GlobalConfig` 中已有 Bark Key / Server / Group，并在 `global_configs` 增加提醒开关、提前分钟、强度和最后发送标记。
  - Why: Bark 是用户级全局配置，Alpha 顶部条只需要控制是否提醒和提醒方式，不应重复保存 Bark 密钥。
- Decision: 后端启动轻量循环扫描 Alpha 空投并发送 Bark，而不是依赖前端页面定时器。
  - Why: 用户关闭页面后仍应按配置收到提醒；后端可以使用最后发送标记避免重复推送。
...
- 外部 Alpha 时间缺少时区信息，可能导致提醒时间偏差。
  - Mitigation: 按服务端本地时区解析 `date time`；本项目当前部署时间区为 Asia/Shanghai，符合用户当前使用语境。
