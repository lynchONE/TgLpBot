## 1. Implementation
- [ ] 1.1 新增 `wallet-swap` spec delta，定义多提供商报价、手续费净值口径、按 provider 控制路径展示和 provider 执行行为
...
- [ ] 1.5 前端：改造 `webapp` `SwapPanel` / `api.js`，展示多 provider 报价卡片、费用和选择态；仅展示 OKX 路径，不展示 0x / LI.FI 路径
