...
func (m *V3PositionManager) BalanceOf(opts *bind.CallOpts, owner common.Address) (*big.Int, error) {
	var result []interface{}
	if err := m.contract.Call(opts, &result, "balanceOf", owner); err != nil {
		return nil, err
	}
	if len(result) < 1 {
		return nil, fmt.Errorf("unexpected balanceOf return length: %d", len(result))
	}
	v, ok := result[0].(*big.Int)
	if !ok || v == nil {
		return nil, fmt.Errorf("unexpected balanceOf type: %T", result[0])
	}
	return v, nil
}

func (m *V3PositionManager) TokenOfOwnerByIndex(opts *bind.CallOpts, owner common.Address, index *big.Int) (*big.Int, error) {
	var result []interface{}
	if err := m.contract.Call(opts, &result, "tokenOfOwnerByIndex", owner, index); err != nil {
		return nil, err
	}
	if len(result) < 1 {
		return nil, fmt.Errorf("unexpected tokenOfOwnerByIndex return length: %d", len(result))
	}
	v, ok := result[0].(*big.Int)
	if !ok || v == nil {
		return nil, fmt.Errorf("unexpected tokenOfOwnerByIndex type: %T", result[0])
	}
	return v, nil
}

func (m *V3PositionManager) OwnerOf(opts *bind.CallOpts, tokenId *big.Int) (common.Address, error) {
	var result []interface{}
	...
	if b, ok := result[0].([20]byte); ok {
		return common.BytesToAddress(b[:]), nil
	}
	return common.Address{}, fmt.Errorf("unexpected ownerOf type: %T", result[0])
}
...
func (m *V3PositionManager) Positions(opts *bind.CallOpts, tokenId *big.Int) (*V3PositionInfo, error) {
	var result []interface{}
	if err := m.contract.Call(opts, &result, "positions", tokenId); err != nil {
		return nil, err
	}
