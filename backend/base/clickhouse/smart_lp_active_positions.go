...
func (s *ClickHouseService) loadSmartLPActivePositionStates(ctx context.Context, keys []string) (map[string]smartLPActivePositionState, error) {
	...
	q := fmt.Sprintf(`
		SELECT
			position_key,
			argMax(chain, tuple(last_event_seq, updated_at)) AS chain,
			...
			argMax(current_liquidity, tuple(last_event_seq, updated_at)) AS current_liquidity,
			...
			argMax(last_remove_at, tuple(last_event_seq, updated_at)) AS last_remove_at,
			max(last_event_seq) AS last_event_seq,
			argMax(last_event_block, tuple(last_event_seq, updated_at)) AS last_event_block,
			argMax(last_event_ts, tuple(last_event_seq, updated_at)) AS last_event_ts,
			argMax(source, tuple(last_event_seq, updated_at)) AS source,
			argMax(updated_at, tuple(last_event_seq, updated_at)) AS updated_at
		FROM %s
