...
func (b *Bot) renderSmartMoneyMessage(ctx context.Context, smCtx *smartMoneyMessageCtx, expandedPoolIdx int, walletPage int) (string, any, int, int, error) {
	...
	if expandedPoolIdx >= 0 && expandedPoolIdx < len(smCtx.Pools) {
		...
		walletMap := make(map[string][]services.SmartLPEvent)
...
func sortEventsInPlace(evs []services.SmartLPEvent) {
	if len(evs) < 2 {
		return
	}
...
func smartMoneyEstimateStableValue(task *models.StrategyTask, ev *services.SmartLPEvent, currentTick int, decimalsCache map[string]int) (float64, string, bool) {
	if task == nil || ev == nil {
		return 0, "", false
	}
	...
	price, _, quoteSym, okPrice := services.BuildPriceDisplay(task, currentTick)
	if !okPrice || price <= 0 {
		return 0, "", false
	}

	side := services.StableSideFromTask(task)
	switch side {
	case 0:
...
func (b *Bot) smartMoneyFormatEventLine(task *models.StrategyTask, ev *services.SmartLPEvent, decimalsCache map[string]int, tickCache map[uint64]int) string {
	if task == nil || ev == nil {
		return ""
	}
	...
	lowerStablePct, upperStablePct := services.StablePercentagesFromTickPercentages(task, lowerTickPct, upperTickPct)
	...
	txShort := shortenHex(strings.TrimSpace(ev.TxHash))
	if txShort == "" {
		txShort = "-"
	}

	return fmt.Sprintf("%s ｜ %s ｜ tx `%s` ｜ block %d", amountText, rangeText, txShort, ev.BlockNumber)
}
