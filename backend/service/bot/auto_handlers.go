...
*怎么选池子*
 ...
 • 对通过硬筛的池子计算 Z5（短期状态）+ 趋势信号（默认启用：MA5/MA60 均线差），并按评分选 Top（评分优先看手续费率，其次参考 5m 手续费与 TVL；共振需满足费用率/成交量/|Z60|阈值）

*怎么制定开仓策略*
 • 候选开仓：状态机（基于 Z5）为 急涨 / 震荡 / 温和上涨
 • 进场门禁（可关闭）：趋势为下跌（MA5 明显低于 MA60）或短期回落（P 低于 MA5）时不作为候选（避免“下跌仍开仓”）
...
func (b *Bot) autoStatsText(user *models.User, cfg *models.AutoLPUserConfig) string {
	...
	stats, err := auto_lp.NewAutoLPStatsService().GetUserStats(user.ID, cfg)
