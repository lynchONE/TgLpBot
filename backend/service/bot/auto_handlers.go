...
• 对通过硬筛的池子计算 Z5 / Z60，并按评分选 Top（评分优先看手续费率，其次参考 5m 手续费与 TVL；共振需满足费用率/成交量/|Z60|阈值）
...
*何时撤仓（自动任务）*
• 触发 1：5m 成交量较开仓时下跌 >=%.0f%%（无时间窗口限制）；首次触发不撤，下一次监测若继续下降才撤
• 触发 2：价格与交易笔数较开仓时下跌 >=%.0f%%（无时间窗口限制）；首次触发不撤，下一次监测仍满足阈值才撤
