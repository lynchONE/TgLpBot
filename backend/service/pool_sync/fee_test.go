...
func TestNormalizePoolSyncFeeKeepsValidV4DynamicFeePercentage(t *testing.T) {
	t.Parallel()

	tier, pct, dynamic := normalizePoolSyncFee("v4", 0x800000, 4.996)
	if !dynamic {
		t.Fatal("expected v4 dynamic fee flag to be marked dynamic")
	}
	if tier != 0 || pct != 4.996 {
		t.Fatalf("dynamic fee output = tier %d pct %.4f, want 0/4.996", tier, pct)
	}
}
